...
module.exports = function validateRegisterInput(data) {
    ...
    if(!Validator.isLength(data.sex, { min: 2, max: 30 })) {
        errors.sex = 'sex must be between 2 to 30 chars';
    }
    
    if(Validator.isEmpty(data.sex)) {
        errors.sex = 'Sex field is required';
    }
    if(!Validator.isLength(data.mobile, { min: 2, max: 30 })) {
        errors.mobile = 'Name must be between 2 to 30 chars';
    }
